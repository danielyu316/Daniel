...
RUN DUMMY=${DUMMY}; \
  ...
  elif [ "$app" = "Codoon" ] ; then \
  python3 run_page/codoon_sync.py ${codoon_phone_number} ${codoon_password};\
  else \
  echo "Unknown app" ; \
  fi
RUN python3 run_page/gen_svg.py --from-db --title "Daniel running Over 10km" --type grid --athlete "Daniel Yu" --output assets/grid.svg --min-distance 10.0 --special-color yellow --special-color2 red --special-distance 20 --special-distance2 40 --use-localtime \
  && python3 run_page/gen_svg.py --from-db --title "Daniel running " --type github --athlete "Daniel Yu" --special-distance 10 --special-distance2 20 --special-color yellow --special-color2 red --output assets/github.svg --use-localtime --min-distance 0.5 \
